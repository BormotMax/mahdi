...
export const AddNewTravelPolicy: FC<Props> = ({ isOpened, onClick }) => {
  ...
  return (
    ...
    <div className={`absolute top-0 left-0 w-full h-full bg-black ${isOpened ? 'bg-opacity-0 hidden' : 'bg-opacity-50'}`}>
      ...
      <form
        className={`w-2/3 h-screen absolute top-0 right-0 bg-white overflow-y-scroll transform ${isOpened ? 'translate-x-full' : ''} duration-1000 ease-in`}
        onSubmit={handleSubmit(onSubmit)}>
        <div className="flex py-11 pl-24 pr-12 justify-between bg-secondary rounded-tl-3xl mb-11">
